...
#   2) generating a random linear system over GF(q) with a planted solution V,
#   3) performing the algorithm’s reduction steps,
#   4) running the meet-in-the-middle style collision search,
#   5) counting (raw) collisions and “filtered” collisions, and checking whether
...
# - The script uses:
#     * Sage’s finite field + linear algebra (GF, random_matrix, rref, vector, matrix)
...
# ----------------------------------------------------------------------------
# Lexicographic subset rank. In paper it is denoted by 𝜌.
# ----------------------------------------------------------------------------
...
# “Guessed/marked” overlap sizes (μ₁, μ₂)
mu1 = 2
mu2 = 2
...
#   - row corresponds to an ℓ-subset s1
#   - col corresponds to an r-subset s2
#   - s1 and s2 are disjoint (s2 chosen from indices not in s1)
#
# This accelerates the “filtering” step: after a collision, we check that
...
for iteration in range(1, 1001):
    ...
    # then union with LMD1 to get an ℓ-subset support.
    for s in Subsets(complement, ell - mu1):
        ...
        #    Δ = (u_value - v_value) * (M1.col(j) - M1.col(j+1)).
        #
        # Here keys are (u_value, v_value, j) storing that Δ.
        precomputed = {}
    ...
    #   - then filter by disjointness (Compatibility Table)
    ...
    #     (i) expected number of repeatition of finding disjoint subsets that 
